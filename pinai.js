...
class Pinai {
  constructor() {
    ...
    this.tokens = require("./token.json");
  }
      ...
      if (response.status === 200) {
        ...
        for (const coin of coins) {
          ...
          if (coin.count > 0) {
            ...
            await this.collectCoins(token, coin);
          }
        }
        ...
        this.log(`Lỗi khi lấy dữ liệu từ API home: ${response.statusText}`, "error");
      }
    } catch (error) {
  ...
  async claimTask(token, taskId, successMessage) {
  ...
  async chekin(token) {
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        if (task?.checkin_detail?.is_today_checkin == 0) await this.completeTask(token, task?.task_id, `Điểm danh hàng ngày thành công | Streak: ${task?.checkin_detail?.consecutive_days + 1}`);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        for (const task of tasks) {
          ...
          if (task.can_claim) {
            await this.claimTask(token, task.task_id, `Claim nhiệm vụ ${task.task_id} | ${task.task_name} thành công`);
          } else {
            await this.completeTask(token, task.task_id, `Làm nhiệm vụ ${task.task_id} | ${task.task_name} thành công | Phần thưởng: ${task.reward_points}`);
          }
        }
      } else {
        this.log(`Lỗi khi lấy danh sách nhiệm vụ: ${response.statusText}`, "error");
      }
    } catch (error) {
  ...
  async main() {
    ...
    while (true) {
      for (let i = 0; i < data.length; i++) {
        ...
        await this.getHomeData(token);
        await this.chekin(token);
        await sleep(3);
